"""Concrete repository implementations for the products bounded context."""
...
class DjangoProductReadRepository(ProductReadRepository):
    """Read-side product queries backed by Django ORM."""
...
class DjangoProductWriteRepository(ProductWriteRepository):
    """Write-side product operations backed by Django ORM."""
    ...
    def save(
        ...
        orm = ProductORM.objects.select_related("brand").prefetch_related("categories").get(
            pk=orm.pk
        )
        return _orm_to_domain(orm)
...
class DjangoBrandRepository(BrandRepository):
    """Brand repository backed by Django ORM."""
...
class DjangoCategoryRepository(CategoryRepository):
    """Category repository backed by Django ORM."""
